...
class SkfRSA(QWidget):


    # RSA签名
    def SKF_RSASignData(self):
        ...
        ulSignDataLen = 32
        g.pSignature = Arr128()
        code = gm.SKF_RSASignData(g.phContainer,byref(pSignData),ulSignDataLen, g.pSignature)
        if code == 0:
            g.textBrowser.append(Message.RSA_SING_DATA_SUCCESS + code_to_str(code))
            logger.info(Message.RSA_SING_DATA_SUCCESS + code_to_str(code))
            pSignatureStr = ""
            print("RSA 签名值 长度  = %s" % len(g.pSignature))
            for i in range(len(g.pSignature)):

                pSignatureStr = pSignatureStr + str((g.pSignature)[i])
            print("SKF_RSASignData pSignatureStr =%s" % (pSignatureStr))
        else:
            g.textBrowser.append(Message.RSA_SING_DATA_FAILED + code_to_str(code))
            logger.error(Message.RSA_SING_DATA_FAILED + code_to_str(code))


    # RSA 验签
    def SKF_RSAVerify(self):
        # 导出签名公钥
        PUBK = Arr128()
        pulBlobLen = c_uint(128)
        bSignFlag = True
        code = gm.SKF_ExportPublicKey(g.phContainer, bSignFlag, PUBK, byref(pulBlobLen))
        if code == 0:
            ...
            print("公钥信息：%s"%bytes_to_list(PUBK))
        else:
        ...
        g.pBlob = RSAPUBLICKEYBLOB()
        g.pBlob.AlgID = SGD_RSA
        g.pBlob.BitLen = 256
        memmove(g.pBlob.Modulus, byref(PUBK, 8), 128)
        memmove(g.pBlob.PublicExponent, byref(PUBK, 136), 4)

        code = gm.SKF_RSAVerify(g.phDev, byref(g.pBlob), HASH, 32, g.pSignature,)
        if code == 0:
            g.textBrowser.append(Message.RSA_VERIFY_SUCCESS + code_to_str(code))
            logger.info(Message.RSA_VERIFY_SUCCESS + code_to_str(code))
        else:
            g.textBrowser.append(Message.RSA_VERIFY_FAILED + code_to_str(code))
            logger.error(Message.RSA_VERIFY_FAILED + code_to_str(code))
    ...
    def SKF_ImportSessionKey(self):
        ...
        # bSignFlag = False
        bSignFlag = True  # 2.0.6 true代表加密公钥与3.0.7相反
    ...
    def SKF_GenerateKeyWithECC(self):
        ...
        code = gm.SKF_GenerateKeyWithECC(phAgreementHandle, byref(reponseECCPubKeyBlob),byref(Agreement_slaveTempPubkey), Agreement_slaveID, 32, byref(g.SessionKey))
        if code == 0:
